...
ResidualModelContactControlGravTpl<Scalar>::ResidualModelContactControlGravTpl(boost::shared_ptr<StateMultibody> state,
                                                                               const std::size_t nu)
    : Base(boost::make_shared<StateMultibody>(*state), state->get_nv(), nu, true, false, true), pin_model_(*(state->get_pinocchio())) {
  if (nu_ == 0) {
...
template <typename Scalar>
ResidualModelContactControlGravTpl<Scalar>::ResidualModelContactControlGravTpl(boost::shared_ptr<StateMultibody> state)
  : Base(boost::make_shared<StateMultibody>(*state), state->get_nv(), state->get_nv(), true, false, true), pin_model_(*(state->get_pinocchio())) {}
